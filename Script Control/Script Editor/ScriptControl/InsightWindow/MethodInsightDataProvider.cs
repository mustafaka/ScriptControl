	...
	public class MethodInsightDataProvider : IInsightDataProvider
	{
		...
		public bool CaretOffsetChanged()
		{
			...
			if (!closeDataProvider) {
				...
				for (int offset = initialOffset; offset < Math.Min(textArea.Caret.Offset, document.Text.Length); ++offset) {
                    char ch = (char)document.Text.Substring(offset, 1).ToCharArray(0,1)[0];
					switch (ch) {
						case '\'':
